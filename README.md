# DojinHakusho
DLsiteをクロールし，任意のクリエイターの統計情報を収集・整理するフリーソフトです． 

主に同人音声作品での使用を想定しています．
...
ここでクリエイター名とはサークル名と声優名を指します．

クリエイター名は文字列で指定され，DLsite上で用いられるクリエイタータグと一致する必要があります．
...
ツールは生データを更に解析し，指定されたクリエイターに関する図表を自動生成します．
図表には売り上げのヒストグラムやジャンルの共起関係などが含まれます．
...
## 利用規約
[本ツールはDLsite様の規約](https://www.dlsite.com/robots.txt)を遵守し，urllib.robotparserのcan_fetchによってアクセスの可否を判断しています．しかしながら，仮に本ツールによってクリエイター様，もしくは株式会社エイシス様に不利益が生じる場合，製作者は速やかに本ツールの修正，もしくは削除を行います．

本ツールはDLsite様を利用するクリエイター様を支援することを目的に開発されたツールです．
...
（特に，robots.txtに違反するような改造は厳禁とします．）
...
## 動作要件
Python3が動作し，ネットワークに接続可能なPCでの利用を想定しています．
...
改良は続けていく予定ですので，ご連絡くだされば検討します．

なお，本ツールは元々オカズ探し目的に作った物を改造しているのでソースは汚いです．そのうち綺麗にします．
